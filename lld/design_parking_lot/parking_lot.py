...
from enums.parking_ticket_status import ParkingTicketStatus
...
class ParkingLot:
    ...
    def is_full(self, type):
        ...
        if type == VehicleType.ELECTRIC:
            ...
            return (self.__compact_spot_count + self.__large_spot_count +
                    self.__electric_spot_count) >= (self.__max_compact_count + self.__max_large_count + self.__max_electric_count)
    ...
    def is_full(self):
        for key in self.__parking_floors:
            if not self.__parking_floors.get(key).is_full(key):
                return False
        return True
